...
const getCartItems = async () => {
  ...
  const { data, error } = await client.from("cart").select(`*, events(*), products(*)`).eq("user_id", user.id);
...
const updateCartItem = async (id: string, updates: Partial<Omit<CartItem, "id" | "created_at">>) => {
  const client = getClient();
  const { data, error } = await client.from("cart").update(updates).select("*").eq("id", id).single();
...
const deleteCartItem = async (id: string) => {
  const client = getClient();
  const { data, error } = await client.from("cart").delete().select("*").eq("id", id);
...
const clearCartItems = async () => {
  ...
  const { data, error } = await client.from("cart").delete().select("*").eq("user_id", user.id);
...
export { createCartItem, getCartItems, updateCartItem, deleteCartItem, clearCartItems };
