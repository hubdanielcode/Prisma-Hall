...
const ProductsTablePagination = ({ currentPage, totalPages, onPageChange }: ProductsTablePaginationProps) => {
  /* - Definições - */
        ...
        {pages.map((page) => (
          <motion.button
            className={`flex justify-center items-center w-8 h-8 text-sm font-semibold border rounded-lg transition-colors cursor-pointer 
              ${currentPage === page ? "border-black text-black bg-[#B8860B]" : "border-[#B8860B] text-[#B8860B] bg-black"}`}
            key={page}
