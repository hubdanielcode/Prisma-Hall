...
const CreateProductModal = ({ isOpen, onClose }: CreateProductModalProps) => {
  ...
  const [newProduct, setNewProduct] = useState<
    Omit<ProductProps, "created_at" | "product_id">
  >({
    name: "",
    description: "",
    category: "",
  ...
  const handleAddProductImage = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      const newProductImage = reader.result as string;
      setProductImage(newProductImage);
      setNewProduct((prev) => ({ ...prev, image: newProductImage }));
      e.target.value = "";
    };
  ...
  // 3. Finaliza o produto e insere-o na tabela
  ...
  const handleAddNewProduct = async () => {
    ...
    if (!newProduct.name.trim()) {
      setProductNameError("Adicione um nome válido.");
      return false;
    }

    if (!newProduct.description.trim()) {
      setProductDescriptionError("Adicione uma descrição válida.");
      return false;
    }
          ...
          >
                ...
                >
                  ...
                  <div className="flex items-center bg-[#0A0A0A] border border-[#333] focus-within:border-[#B8860B] rounded-lg px-3 transition-colors">
                    ...
                    <input
                      ...
                      value={productPrice}
                      onChange={(e) => {
                        const maskedPriceString = masks.productPrice(
                          e.target.value,
                        );
                        setProductPrice(maskedPriceString);

                        const maskedPriceNumber = Number(
                          maskedPriceString.replace(",", "."),
                        );
                        setNewProduct((prev) => ({
                          ...prev,
                          price: Number.isNaN(maskedPriceNumber)
                            ? 0
                            : maskedPriceNumber,
                        }));
                      }}
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <div className="flex items-center justify-between rounded-lg px-3 py-2.5">
                  ...
                  <button
                    ...
                    onClick={() => {
                      const updateStatus =
                        newProduct.status === "Ativo" ? "Inativo" : "Ativo";
                      setNewProduct((prev) => ({
                        ...prev,
                        status: updateStatus,
                      }));
            ...
            <div className="flex flex-wrap justify-end gap-3 px-5 py-4 border-t border-[#B8860B60]">
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                onClick={async () => {
                  const success = await handleAddNewProduct();
                  if (success) {
                    onClose();
                  }
                }}
