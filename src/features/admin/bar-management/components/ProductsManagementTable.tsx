...
const ProductsManagementTable = ({ currentPage, onPageChange }: ProductsManagementTableProps) => {
  ...
  const { setProductBeingEdited, setProductBeingDeleted } = useProductsContext();
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-3 text-white">
          {paginatedProducts.map((product) => {
            const badge =
              productCategoryBadges[product.category as keyof typeof productCategoryBadges];
        ...
        <motion.table className={`w-full ${isLandscapeMobile ? "min-w-160]" : ""}`}>
          ...
          >
            <tr className={`border-b border-[#B6880660] ${isLandscapeMobile ? "h-16" : "h-20"}`}>
              ...
              <th className={`text-center ${isLandscapeMobile ? "px-3 py-2" : "px-4 py-2"}`}>
                Categoria
              </th>
              ...
              return (
                ...
                >
                    ...
                    <div className={`flex gap-1 ${isLandscapeMobile ? "p-2" : "p-3"}`}>
                      <img
                        className={`object-cover border border-[#B8860B] rounded-lg ${
                          isLandscapeMobile ? "h-12 w-12 mx-2" : "h-16 w-16 mx-4"
                        }`}
                        src={product.image}
                        alt={product.name}
                      />
                  ...
                  <td className="text-center text-[#B8860B]">
                    <span className={`font-bold ${isLandscapeMobile ? "pl-2 pr-1" : "pl-4 pr-1"}`}>
                      R$
                    </span>
