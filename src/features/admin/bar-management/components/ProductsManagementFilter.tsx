...
const ProductsManagementFilter = ({ onPageChange }: ProductsManagementFilterProps) => {
  ...
  const { productsCategories, selectedCategory, setSelectedCategory, searchQuery, setSearchQuery } =
    useBarContext();
  ...
  return (
    <div
      className={`flex w-full gap-4 ${
        isPortraitMobile ? "flex-col" : "flex-row items-center justify-between gap-3"
      }`}
    >
      {/* - Searchbar - */}
