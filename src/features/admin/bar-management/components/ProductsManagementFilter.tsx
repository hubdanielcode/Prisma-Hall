...
const ProductsManagementFilter = ({ onPageChange }: ProductsManagementFilterProps) => {
  ...
  const { categories, selectedCategory, setSelectedCategory, searchQuery, setSearchQuery } = useBarContext();
  const { isPortraitMobile } = useMobileContext();

  /* - Estados de paginação - */

  return (
    <div className={`flex w-full gap-4 ${isPortraitMobile ? "flex-col" : "flex-row items-center justify-between gap-3"}`}>
      {/* - Searchbar - */}
      ...
      <div className={`flex flex-wrap items-center gap-2.5 ${isPortraitMobile ? "justify-start" : "justify-end flex-1 flex-nowrap"}`}>
