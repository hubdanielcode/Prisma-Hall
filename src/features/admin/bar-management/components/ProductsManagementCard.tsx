...
import { useProductsContext } from "@/features/bar";
...
const ProductsManagementCard = () => {
  ...
  const productCardData = {
    ...
    Coquetéis: {
      icon: <Martini />,
      title: "Coquetéis",
      quantity: products.filter((product) => product.category === "Coquetéis").length,
    },
    ...
    Cervejas: {
      icon: <Beer />,
      title: "Cervejas",
      quantity: products.filter((product) => product.category === "Cervejas").length,
    },

    "Sem Álcool": {
      icon: <Coffee />,
      title: "Sem Álcool",
      quantity: products.filter((product) => product.category === "Sem Álcool").length,
    },
  };
    ...
    <div
      className={`w-full ${
        isPortraitMobile
          ...
          : `flex flex-nowrap items-stretch ${isLandscapeMobile ? "gap-3" : "gap-4"}`
      }`}
    >
      ...
      {uniqueCategories.map((category) => {
        const card = productCardData[category as keyof typeof productCardData];
