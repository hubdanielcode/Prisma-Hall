...
const BarManagement = () => {
  ...
  const { productBeingEdited, setProductBeingEdited, productBeingDeleted, setProductBeingDeleted } =
    useProductsContext();
    ...
    <>
      <div
        className={`bg-[#1A1A1A] min-h-screen w-full ${
          isPortraitMobile ? "pb-10" : isLandscapeMobile ? "pb-12 px-6" : "pb-14 px-8"
        }`}
      >
            ...
            <div className={`flex flex-col items-start ${isPortraitMobile ? "mb-4" : "mb-0"}`}>
              <span
                className={`text-white font-semibold ${
                  isPortraitMobile ? "text-lg" : isLandscapeMobile ? "text-xl" : "text-2xl"
                }`}
              >
                Gestão do Bar
              </span>
