...
const createReview = async (review: Omit<ReviewProps, "created_at" | "review_id" | "user_id">) => {
...
const getReviews = async () => {
  ...
  const { data, error } = await client.from("reviews").select(`*, events (title), users (name, photo)`);
...
const getSingleReview = async (review_id: string) => {
  ...
  const { data, error } = await client.from("reviews").select(`*, events (title), users (name, photo)`).eq("review_id", review_id).single();
...
const updateReview = async (review_id: string, updates: Partial<Omit<ReviewProps, "created_at" | "review_id" | "user_id">>) => {
  const client = getClient();

  const { data, error } = await client.from("reviews").update(updates).select("*").eq("review_id", review_id).single();
...
const deleteReview = async (review_id: string) => {
  ...
  const { data, error } = await client.from("reviews").delete().eq("review_id", review_id).select("*").single();
...
export { createReview, getReviews, getSingleReview, updateReview, deleteReview };
