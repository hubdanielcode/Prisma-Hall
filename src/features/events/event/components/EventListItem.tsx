...
const EventListItem = ({ event, onSelect }: EventListItemProps) => {
  return (
    <motion.button
      className="flex items-center gap-4 w-full bg-black border border-[#B8860B60] hover:border-[#B8860B] rounded-lg px-5 py-2.5 cursor-pointer transition-colors text-left"
    ...
    >
      ...
      <span className="text-[#B8860B] font-bold text-sm sm:text-base shrink-0">R$: {event.price.toFixed(2).replace(".", ",")}</span>
    </motion.button>
  );
};
