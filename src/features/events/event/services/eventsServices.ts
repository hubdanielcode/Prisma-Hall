...
const checkIsAdmin = async () => {
  ...
  const { data, error } = await client.from("users").select("role").eq("user_id", user.id).single();
...
const updateEvent = async (event_id: string, updates: Partial<Omit<EventProps, "created_at" | "id">>) => {
  await checkIsAdmin();
  const client = getClient();

  const { data, error } = await client.from("events").update(updates).eq("event_id", event_id).select("*").single();
...
const deleteEvent = async (event_id: string) => {
  ...
  const { data, error } = await client.from("events").delete().eq("event_id", event_id).select("*").single();
...
export { checkIsAdmin, createEvent, getEvents, getSingleEvent, updateEvent, deleteEvent };
