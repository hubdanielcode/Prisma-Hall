...
const CustomPasswordInput = ({ className, label, placeholder, value, onChange, maxLength }: PasswordInputProps) => {
  ...
  return (
    <div className="flex flex-col">
      ...
      <span className="text-sm text-white/60 font-semibold mb-1 mt-2 md:mb-2 md:mt-4 capitalize">{label}</span>

      {/* - Input - */}

      <div className={`flex w-full rounded-lg px-4 py-2 bg-[#1A1A1A] ${className}`}>
        ...
        >
          {isPasswordPrivate ? <Eye className="h-5 w-5" /> : <EyeClosed className="h-5 w-5" />}
        </button>
      </div>

      <span className="text-sm text-white/60 ml-2 mt-1 mb-3">Mínimo de 6 caracteres</span>
    </div>
  );
};
