...
const AppContent = () => {
  ...
  const { session, setSession, setIsAuthenticated } = useAuthenticationContext();
  ...
  useEffect(() => {
    ...
    const { data: supabaseTempAuthListener } = supabaseTemp.auth.onAuthStateChange(
      (e: AuthChangeEvent, session: Session | null) => {
        if (e === "SIGNED_OUT") navigate("/", { replace: true });
        setSession(session);
        setIsAuthenticated(!!session);
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col justify-center items-center max-w-full min-h-screen bg-[url(/assets/images/ph-quinas.png)] bg-cover bg-center">
        <motion.span
